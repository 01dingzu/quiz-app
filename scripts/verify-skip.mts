...
console.log('\n[2] 已答题不可跳过')
fresh()
setState({ session: [q1, q2], index: 0, picked: { [q1]: { t: 'single', k: 'A' } } })
useQuiz.getState().skipCurrent()
...
useQuiz.getState().submitAnswer(q1, { t: 'single', k: BANK[0].answer })
...
fresh()
setState({ session: [q1, q2, q3, q4], index: 3, picked: { [q1]: { t: 'single', k: 'A' } }, skipped: [q1, q3] })
...
fresh()
setState({ session: [q1, q2, q3], index: 2, picked: { [q1]: { t: 'single', k: 'A' }, [q2]: { t: 'single', k: 'A' } }, skipped: [q1] })
useQuiz.getState().resumeSession()
...
fresh()
setState({ session: [q1, q2, q3], index: 1, picked: { [q1]: { t: 'single', k: 'A' } }, skipped: [] })
useQuiz.getState().resumeSession()
check('会话顺序不变', JSON.stringify(getState().session) === JSON.stringify([q1, q2, q3]), getState().session)
...
fresh()
setState({ session: [q1, q2, q3], index: 0, picked: { [q1]: { t: 'single', k: 'A' }, [q2]: { t: 'single', k: 'A' }, [q3]: { t: 'single', k: 'A' } }, skipped: [q2] })
useQuiz.getState().resumeSession()
check('会话顺序不变', JSON.stringify(getState().session) === JSON.stringify([q1, q2, q3]), getState().session)
check('index 不动', getState().index === 0, getState().index)
